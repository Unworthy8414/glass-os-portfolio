...
import { SectionHeader, PersonaCard } from '../components';
...
export const AnalysisSection = ({ data, isTimeMgmt }: SectionProps) => (
  <motion.section
    ...
    viewport={{ once: true }}
  >
    <SectionHeader number="03" title="Insights" color={colors.jasmine} icon={Lightbulb} />
    ...
    <div className="space-y-8">
      ...
      <div>
        <motion.h3
          ...
          viewport={{ once: true }}
        >
          {isTimeMgmt ? 'Key Insights' : 'Key Findings'}
        </motion.h3>
          ...
          {isTimeMgmt && isTimeManagementData(data) ? (
            data.keyFindings.map((finding, i) => (
              <motion.div
                ...
                whileHover={{ borderColor: `${colors.charcoal}66`, y: -2 }}
              >
                <h4 className="font-medium mb-2" style={{ color: colors.charcoal }}>{i + 1}. {finding.title}</h4>
      ...
      {!isTimeMgmt && isAGODigitalData(data) && (
        <motion.div
          ...
          style={{ backgroundColor: `${colors.verdigris}1A`, borderColor: `${colors.verdigris}33` }}
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          ...
          >
            <Sparkles size={16} style={{ color: `${colors.verdigris}80` }} />
          </motion.div>
          <h4 className="font-medium mb-2 flex items-center gap-2" style={{ color: colors.verdigris }}>
