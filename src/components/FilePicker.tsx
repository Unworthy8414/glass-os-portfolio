...
import { Folder, FileText, Image as ImageIcon, HardDrive, Clock, Cloud, Download, Monitor } from 'lucide-react';
...
export const FilePicker: React.FC<{ windowId: string }> = ({ windowId }) => {
    ...
    const { closeWindow, launchApp, windows, updateWindowProps } = useOSStore();
    const [currentPath, setCurrentPath] = useState('desktop');
    const [selectedFileId, setSelectedFileId] = useState<string | null>(null);
    
    // Get the target window ID from this picker window's props (if passed)
    ...
    const handleSelect = (file: any) => {
        ...
        let appId = 'editor';
        if (file.kind === 'pdf') appId = 'pdf';
        else if (file.kind === 'image') appId = 'photos';
        
        // Check if we have a target window and if it matches the file type
        if (targetWindowId) {
            ...
            <div className="flex-1 flex flex-col bg-[#1e1e1e]/50 overflow-hidden">
                ...
                <div className="flex-1 p-4 grid grid-cols-4 gap-4 content-start overflow-auto">
                    ...
                    {items.map((item) => (
                        ...
                        >
                             {item.kind === 'pdf' ? <FileText size={32} strokeWidth={1} className="text-red-400 drop-shadow-lg" /> :
                             item.kind === 'image' ? <ImageIcon size={32} strokeWidth={1} className="text-purple-400 drop-shadow-lg" /> :
                             item.kind === 'python' ? <PythonIcon size={32} className="drop-shadow-lg" /> :
                             item.kind === 'text' ? <FileText size={32} strokeWidth={1} className="text-gray-400 drop-shadow-lg" /> :
                             <Folder size={32} strokeWidth={1} className="text-blue-400 fill-blue-400/20 drop-shadow-lg" />}
